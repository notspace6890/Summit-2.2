...
public class CapeManager implements EventListener
{
    ...
    public CapeManager() {
        ...
        this.OnGetLocationSkin = new Listener<EventPlayerGetLocationCape>(p_Event -> {
            if (Wrapper.GetMC().field_71446_o != null) {
                if (this.CapeUsers.containsKey(p_Event.Player.func_70005_c_())) {
                    p_Event.cancel();
                    p_Event.SetResourceLocation(this.CapeUsers.get(p_Event.Player.func_70005_c_()));
                }
            }
        });
    }
    ...
    public void DownloadCapeFromLocationWithName(final String p_Link, final String p_Name) throws MalformedURLException, IOException {
        ...
        this.Capes.put(p_Name, Wrapper.GetMC().func_110434_K().func_110578_a("salhack/capes", l_Texture));
    }
